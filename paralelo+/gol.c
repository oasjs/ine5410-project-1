...
void initialize_game(pthread_barrier_t* wait, pthread_barrier_t* start, int steps)
{
    barrier_wait = wait;
    barrier_start = start;
    steps_ = steps;
...
void* play(void* arg)
{
    ...
    int numberOfLines = args.n_lines + firstLine;

    for (int step = 0; step < steps_; step++)
    {
        /* for each cell, apply the rules of Life */
        for (i = firstLine; i < numberOfLines; i++)
        {
            for (j = 0; j < size; j++)
            {
        ...
        // Espera a troca de tabuleiros na função main
